...
export default class JsonFormFill extends Component {
  ...
  validateForm() {
    ...
    const {omitFieldsByLabel} = this.props;
    const form = [...jsonForm.filter(field => (
      (omitFieldsByLabel || []).findIndex(label => label === field.label) === -1 
    )), ...customFields];
    ...
    return (
      <div>
        <Row>
          ...
          <Col span={24}>
            {
              jsonForm && jsonForm.reduce((array, field, index) => (
                (this.props.omitFieldsByLabel || []).find(item => field.label === item) ?
                array
                :
                [...array, this[field.type](field, index, 'jsonForm')]
              ),[])
            }
          </Col>
        </Row>
